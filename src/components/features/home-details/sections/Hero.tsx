...
export const Hero = async ({ movie }: IHeroProps) => {
  ...
  return (
    <Paper radius={0} pos="relative">
      <BackgroundImage src={`${env.NEXT_PUBLIC_IMAGE_URL}/original${movie.backdrop_path}`}>
        <Container size="xl" px="md" py={20} pos="relative" style={{ zIndex: 2 }}>
          <BackButton />
          <Group align="flex-start" mt="md">
            ...
            <Stack gap={8} w={{ base: '100%', sm: '70%' }}>
              <Title c="white" fz={{ base: 30, sm: 50 }} fw={600}>
                {movie.original_title} ({dayjs(movie.release_date).locale(locale).format('YYYY')})
              </Title>
              <Group gap="sm">
